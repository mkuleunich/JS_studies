...
let parsedJson = testJson.list.entries.map(item => {
    return {docId: 'http://doc.epam.com/' + item.entry.name.replace('.html', '')};
});
fs.writeFile('./tasks/json/parsed.json', JSON.stringify(parsedJson), (err) =>
{
    if (err) {
        throw err;
    }
